<div class="bg-white rounded-[3rem] shadow-2xl border border-slate-100"
     x-data="{
        ...
        get topPekerjaanAktif() {
            ...
            let namaPekerjaan = '-';
            this.itemList.forEach(nama => {
                if (nama.toLowerCase().includes('belum') || nama.toLowerCase().includes('tidak')) return;
                const data = this.allYearsData[nama][this.selectedTahun];
                if (data && data.total > maxTotal) { maxTotal = data.total; namaPekerjaan = nama; }
            });
            return { nama: namaPekerjaan, jumlah: maxTotal };
        },

        // Fungsi Sinkronisasi Klik
        ...
        initChart() {
            ...
            if (this.selectedItem !== 'Semua') {
                // JIKA SATU PEKERJAAN DIPILIH: Belah jadi LK & PR
                labels = ['Laki-laki', 'Perempuan'];
                dataValues = [this.currentStats.lk, this.currentStats.pr];
                bgColors = ['#2563eb', '#db2777'];
            } else {
                // JIKA SEMUA: Tampilkan daftar semua pekerjaan
                ...
                options: {
                    ...
                    onClick: (e, el) => {
                        if (el.length > 0) {
                            ...
                            if (self.selectedItem === 'Semua') {
                                self.selectIndicator(clickedName = clickedLabel);
                            } else {
                                self.selectedItem = 'Semua';
                            }
                        }
                    },
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: (c) => `${c.label}: ${c.raw.toLocaleString('id-ID')} jiwa`
                            }
                        }
                    },
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 p-10 items-start">
        <div class="space-y-6">
            ...
            <div class="flex justify-between items-center mb-4">
                <h3 @click="selectedItem = 'Semua'" class="text-lg font-black uppercase italic text-slate-700 cursor-pointer hover:text-blue-600 transition-all">
                    Detail Mata Pencaharian <span x-show="selectedItem !== 'Semua'" class="text-xs text-red-500 animate-pulse ml-2">(RESET)</span>
                </h3>
        ...
        <div class="lg:sticky lg:top-6 self-start h-fit flex flex-col items-center">
            ...
            <div class="w-full mb-8 bg-white rounded-[2rem] p-4 border border-slate-100 shadow-sm h-[450px] flex items-center justify-center">
